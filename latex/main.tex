...
Bài tiểu luận trình bày hệ thống giám sát và điều khiển môi trường trang trại thông minh trên nền ESP32, MQTT và Node-RED Dashboard, đúng với mã nguồn \file{main.cpp}, flow \file{node-red.json} và sơ đồ mô phỏng \file{diagram.json} đính kèm. Cấu trúc báo cáo gồm bảy chương chính (mục lục): (1)~Đặt vấn đề; (2)~Nội dung thực hiện; (3)~Thiết kế mạch IoT; (4)~Nguyên lý hoạt động; (5)~Cài đặt hệ thống; (6)~Kết quả thực hiện; (7)~Kết luận và hướng phát triển --- sau đó là tài liệu tham khảo và các phụ lục mã nguồn (theo Phụ lục~II).
...
Trong bối cảnh biến đổi khí hậu và đô thị hóa, tưới tiêu thủ công tại nhà kính, ban công hay sân thượng thường không đảm bảo cung cấp nước đúng lúc; người trồng cần theo dõi nhiệt độ, độ ẩm không khí và độ ẩm đất, đồng thời có khả năng cảnh báo và điều khiển tưới từ xa. Việc tự động hóa giám sát --- truyền dữ liệu lên cloud --- hiển thị trên giao diện web và gửi lệnh điều khiển ngược xuống thiết bị là nhu cầu thực tế và phù hợp với học phần phát triển ứng dụng IoT.

Đề tài hướng tới các mục tiêu kỹ thuật sau, khớp với firmware và flow đã cài đặt:
...
Chương này tóm tắt các chức năng đã cài đặt trên firmware ESP32 (\file{main.cpp}), flow Node-RED (\file{node-red.json}) và sơ đồ mạch mô phỏng (\file{diagram.json}). Hệ thống \textbf{không} dùng OLED, DHT11 hay đồng bộ giờ NTP; cảm biến khí tượng là \textbf{DHT22}; độ ẩm đất mô phỏng bằng biến trở đọc ADC.
...
Khi \code{soil < 2048} và biến flow \code{manual == true} (switch \textit{Tưới tự động}), flow gửi \code{\{"pump":true,"duration":5\}} lên \code{iot/farm/control}. Đây là logic tự động theo \textbf{ngưỡng đất}, không dùng đồng hồ NTP trong bài này.
...
Theo yêu cầu hướng dẫn: sơ đồ logic (EasyEDA), chức năng linh kiện, mạch vật lý/minh họa Wokwi (\file{mach-vat-ly-wokwi.png}, khớp \file{diagram.json}, Phụ lục~C). Ảnh mạch in thực tế có thể thay thế; ghi chú \file{anh-mach-vat-ly-thuc-te.png.txt}.

\subsection{Thành phần phần cứng}

Vi điều khiển ESP32 DevKit C v4; cảm biến DHT22; biến trở mô phỏng độ ẩm đất; LED và điện trở hạn dòng; buzzer cảnh báo. Sơ đồ EasyEDA có thể bao gồm khối mở rộng (OLED, relay) cho hướng phát triển; mạch chạy trong đề tài khớp \file{diagram.json} (DHT22, LED, potentiometer, buzzer).
...
\textbf{ESP32:} SoC Wi-Fi/Bluetooth, CPU Xtensa dual-core, ADC 12-bit; lập trình bằng PlatformIO/Arduino (\file{main.cpp}).

\textbf{DHT22:} Đo nhiệt độ và độ ẩm KK, giao tiếp 1-wire trên GPIO~4; độ chính xác tốt hơn DHT11.

\textbf{Biến trở (độ ẩm đất):} Đọc ADC 0--4095 tại GPIO~34; hiển thị \% trên Node-RED theo \code{(1 - soil/4095)*100}.

\textbf{LED và buzzer:} LED GPIO~5 mô phỏng máy bơm; buzzer GPIO~18 cảnh báo khi \code{soilRaw < 2048}.

\subsection{Luồng hoạt động tóm tắt theo chức năng}

Thu thập $\rightarrow$ xử lý ngưỡng buzzer $\rightarrow$ đóng gói JSON $\rightarrow$ MQTT publish; song song subscribe \code{iot/farm/control} để bật/tắt bơm và \code{checkPumpTimeout}. Chi tiết sơ đồ kiến trúc, luồng dữ liệu và lưu đồ firmware xem chương~\ref{sec:nguyen-ly}.
...
ESP32 đọc cảm biến, gọi \code{publishSensorData()} khi MQTT đã kết nối; broker chuyển payload JSON tới Node-RED; các nhánh gauge/chart/toast hiển thị (mô tả chương~\ref{sec:noi-dung-thuchien}).
...
Điều kiện \code{soil < 2048} và \code{flow.manual == true} kích hoạt gửi lệnh bơm; không dùng lịch giờ hay NTP.
